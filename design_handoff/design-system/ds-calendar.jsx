    ...
    <div>
      <DSSubsection title="Month Calendar">
        <p style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 14, lineHeight: 1.5 }}>
          Interactive month grid with event chips. Click a day to select. Today is highlighted in amber. View mode toggle for month/week/day.
        </p>
        <div className="glass" style={{ borderRadius: 14, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--rule)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontFamily: 'var(--display)', fontWeight: 700, fontSize: 16 }}>April 2026</span>
              <button className="ds-icbtn" style={{ width: 28, height: 28 }}>
            ...
            <div style={{ display: 'flex', gap: 2, background: 'var(--cream-2)', borderRadius: 7, padding: 2 }}>
              {['month', 'week', 'day'].map(m => (
                <button key={m} className={`ds-filter-pill ${viewMode === m ? 'active' : ''}`} onClick={() => setViewMode(m)} style={{ textTransform: 'capitalize' }}>{m}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
            {weekDays.map(d => (
              <div key={d} style={{ padding: '8px', textAlign: 'center', fontFamily: 'var(--mono)', fontSize: 9.5, color: 'var(--ink-4)', letterSpacing: '.06em', textTransform: 'uppercase', fontWeight: 600, borderBottom: '1px solid var(--rule)' }}>{d}</div>
            ))}
          </div>
            ...
            {cells.map((d, i) => {
              ...
              return (
                <div
                  ...
                  onMouseOver={e => { if (d && !isSel) e.currentTarget.style.background = 'rgba(0,0,0,.015)'; }}
                  onMouseOut={e => { if (d && !isSel) e.currentTarget.style.background = ''; }}
                >
                  {d && (
                    <>
                      <div style={{
                        ...
                        background: isToday ? 'var(--amber)' : isSel ? 'var(--ink)' : 'transparent',
                      ...
                      {events.map((ev, j) => (
                        <div key={j} style={{
                          ...
                          background: ev.color + '15', borderLeft: `2px solid ${ev.color}`,
      ...
      <DSSubsection title="Mini Calendar + Event List">
        <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 16 }}>
          ...
          <div className="glass" style={{ borderRadius: 14, padding: 14 }}>
            <div style={{ fontFamily: 'var(--display)', fontWeight: 700, fontSize: 13, marginBottom: 10, textAlign: 'center' }}>April 2026</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1, textAlign: 'center' }}>
              {weekDays.map(d => (
                <div key={d} style={{ fontFamily: 'var(--mono)', fontSize: 8, color: 'var(--ink-4)', padding: 2, fontWeight: 600 }}>{d[0]}</div>
              ))}
              {/* offset blanks */}
              {Array.from({ length: startOffset }, (_, i) => <div key={`b${i}`}></div>)}
              {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(d => {
                ...
                return (
                  <div key={d} onClick={() => setSelectedDay(d)} style={{
                    ...
                    fontSize: 10, fontWeight: isSel ? 700 : 400, cursor: 'pointer', position: 'relative', margin: '0 auto',
                    background: isSel ? 'var(--amber)' : 'transparent',
                    color: isSel ? '#fff' : 'var(--ink-2)', transition: 'all .12s',
                  }}>
                    {d}
                    {hasEv && !isSel && <span style={{ position: 'absolute', bottom: 1, width: 3, height: 3, borderRadius: '50%', background: 'var(--amber)' }}></span>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="glass" style={{ borderRadius: 14, padding: 16 }}>
            <div style={{ fontFamily: 'var(--display)', fontWeight: 700, fontSize: 14, marginBottom: 14 }}>Upcoming Events</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {CAL_EVENTS.map((ev, i) => (
                <div key={i} style={{
                  display: 'flex', alignItems: 'center', gap: 12, padding: '8px 10px', borderRadius: 8,
                  borderLeft: `3px solid ${ev.color}`, transition: 'background .12s',
                  background: ev.day === selectedDay ? 'rgba(245,158,11,.05)' : 'transparent',
                }}
                  onMouseOver={e => e.currentTarget.style.background = 'rgba(0,0,0,.02)'}
                  onMouseOut={e => e.currentTarget.style.background = ev.day === selectedDay ? 'rgba(245,158,11,.05)' : ''}
                  onClick={() => setSelectedDay(ev.day)}
                >
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 20, fontWeight: 800, color: 'var(--ink-3)', width: 30, textAlign: 'center' }}>{ev.day}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 12.5 }}>{ev.label}</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-4)', marginTop: 1 }}>Apr {ev.day} · {ev.time}</div>
                  </div>
                </div>
              ))}
            </div>
