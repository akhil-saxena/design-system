		...
		>
			<div
				style={{
					...
					transform: active ? "translateX(164px)" : "translateX(0)",
				}}
			/>
		</div>
	);
		...
		<div>
			<DSSubsection title="Motion Tokens">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 14, lineHeight: 1.5 }}>
					A small set of named easings and durations keeps motion coherent. Pair an easing with a
					purpose — entries decelerate, exits accelerate, toggles meet in the middle.
				</p>
					...
					>
						{DURATIONS.map((d) => (
							<div
								key={d.name}
								className="glass"
								style={{ borderRadius: 10, padding: 12, textAlign: "center" }}
							>
								<div
									style={{
								...
								<div
									style={{ fontFamily: "var(--mono)", fontSize: 10, fontWeight: 700, marginTop: 6 }}
								>
									{d.name}
								</div>
							...
							<div>
								...
								<div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 12 }}>
									{EASINGS.map((e) => (
										<button
											key={e.name}
											className={`ds-btn ${easing === e.value ? "amber" : ""}`}
								...
								<div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
									{DURATIONS.map((d) => (
										<button
											key={d.name}
											className={`ds-btn ${duration === d.value ? "amber" : ""}`}
											style={{ fontSize: 11, padding: "4px 8px" }}
