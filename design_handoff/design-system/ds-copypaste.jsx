	...
	return (
		<button
			...
			style={{
				...
				borderRadius: 6,
				border: "1px solid var(--rule)",
				background: copied ? "rgba(34,197,94,.1)" : "var(--surf-3)",
				cursor: "pointer",
				fontSize: 11,
				fontWeight: 600,
				color: copied ? "var(--green)" : "var(--ink-2)",
		...
		<div>
			<DSSubsection title="Click to Copy">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 14, lineHeight: 1.5 }}>
					Tiny gesture, big quality-of-life win. Always confirm visually — a check icon, a label
					flip, a brief color shift — so the user knows the click landed.
				</p>
				...
				<StateRow label="Field with copy button — for URLs and snippets">
					<div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 8 }}>
						...
						<CopyButton
							variant="field"
							value="curl -X POST https://api.jobdash.app/v1/applications -H 'Authorization: Bearer …'"
						/>
					</div>
				</StateRow>
