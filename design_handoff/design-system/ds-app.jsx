...
const SECTIONS = [
	...
	{
		id: "infinitescroll",
		label: "Infinite & Virtualized List",
		icon: "table",
		group: "Data Display",
	},
...
function NavIcon({ type }) {
	...
	switch (type) {
		...
		case "flow":
			return (
				<svg viewBox="0 0 24 24" {...p} style={s}>
					<path d="M5 3v4M3 5h4M6 17v4M4 19h4M13 3l7 7M17 3h3v3M21 17l-7 7m3 0h4v-3" />
				</svg>
			);
		case "upload":
			return (
...
function DSApp() {
	...
	React.useEffect(() => {
		...
		try {
			localStorage.setItem("jobdash-theme", dark ? "dark" : "light");
		} catch {}
	}, [dark]);
	...
	const renderSection = () => {
		switch (section) {
			case "cover":
				return <CoverSection />;
			case "screens":
				return <SampleScreensSection />;
			case "emails":
				return <EmailTemplatesSection />;
			case "mobile":
				return <MobileViewsSection />;
			case "status":
				return <StatusPagesSection />;
			case "tokenexport":
			...
			case "searchauto":
				return <SearchAutocompleteSection />;
			case "datagrid":
				return <DataGridSection />;
			case "calendarview":
				return <CalendarViewSection />;
			case "richtexteditor":
			...
			case "universal":
				return <UniversalSection />;
			case "coachmarks":
				return <CoachMarksSection />;
			case "mediacards":
				return <MediaCardsSection />;
			case "formvalidation":
				return <FormValidationSection />;
			case "breadcrumbs":
			...
			case "typespec":
				return <TypeSpecSection />;
			case "iconset":
				return <IconSetSection />;
			case "illustrations":
				return <IllustrationsSection />;
			case "appbar":
			...
			case "timeline2":
				return <HorizontalTimelineSection />;
			case "infinitescroll":
				return <InfiniteScrollSection />;
			case "bottomsheet":
				return <BottomSheetSection />;
			case "lightbox":
				return <LightboxSection />;
			case "confirmdialog":
				return <ConfirmDialogSection />;
			case "copypaste":
				return <CopyPasteSection />;
			case "hovercard":
				return <HoverCardSection />;
			default:
				return null;
		}
	};
	...
	return (
		<div className="ds-chrome">
			<aside className="ds-sidebar">
				<div className="ds-sidebar-logo">
					<svg viewBox="0 0 210 100" width="28" height="14">
						...
						<g transform="translate(100 20) skewX(-14)">
							...
							<path
								d="M 20 48 Q 20 64 6 64 Q -4 64 -5 54"
								fill="none"
								stroke="var(--amber)"
								strokeWidth="12"
								strokeLinecap="round"
					...
					{[...new Set(SECTIONS.map((s) => s.group))].map((g) => (
						<React.Fragment key={g}>
							...
							{SECTIONS.filter((s) => s.group === g).map((s) => (
								<div
									key={s.id}
									className={`ds-sidebar-item ${section === s.id ? "active" : ""}`}
				...
				<div
					style={{ marginTop: "auto", padding: "12px 14px", borderTop: "1px solid var(--rule)" }}
				>
					<div
						style={{
							fontFamily: "var(--mono)",
					...
					>
						<span
							style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--green-vivid)" }}
						/>
			...
			<main className="ds-main" ref={contentRef}>
				...
				<div className="ds-main-body">{renderSection()}</div>
			</main>
		</div>
	);
}
