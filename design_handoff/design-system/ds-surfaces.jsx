				...
				<div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
					...
					<div
						...
						style={{
							...
							background: "linear-gradient(145deg,rgba(254,243,199,.5),var(--surf-1))",
						}}
					>
						<div
							style={{
						...
						<div style={{ fontSize: 13, color: "var(--ink-2)", lineHeight: 1.5 }}>
							Highlighted card for CTAs or next-actions.
						</div>
					</div>
					<div
						style={{
					...
					>
						...
						<div style={{ fontSize: 13, color: "#aaa39e", lineHeight: 1.5 }}>
							For dark surfaces or inverted contexts.
						</div>
					</div>
				</div>
			</DSSubsection>
			...
			<DSSubsection title="Sticky Note">
				<div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
					<div className="ds-sticky">
						...
						<div className="ds-sticky-hint">Click to expand</div>
					</div>
					<div className="ds-sticky" style={{ transform: "rotate(.4deg)" }}>
						Prep 3 questions for Maya:
						<br />- Division scope
						<br />- Trial expectations
						<br />- Mentorship<div className="ds-sticky-hint">3 items</div>
					</div>
					<div className="ds-sticky" style={{ transform: "rotate(-.8deg)" }}>
						Follow up if no reply by May 2.<div className="ds-sticky-hint">Reminder</div>
					</div>
				</div>
			</DSSubsection>
				...
				<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
					<div className="glass" style={{ borderRadius: 14, overflow: "hidden" }}>
						...
						<div style={{ padding: "16px 20px", fontSize: 13, color: "var(--ink-2)" }}>
							Content area with header bar pattern used throughout the app.
						</div>
					</div>
					<div
						style={{
			...
			<div style={{ display: "flex", gap: 10 }}>
				<div
					style={{
						...
						height: 34,
						borderRadius: 8,
						background: `linear-gradient(145deg,${logoBg},${logoBg})`,
						color: "#fff",
						display: "flex",
						alignItems: "center",
						justifyContent: "center",
				...
				<span
					style={{ fontFamily: "var(--mono)", fontSize: 9.5, color: "var(--ink-4)", marginTop: 2 }}
				>
					{age}
				</span>
			</div>
			<div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 8 }}>
				<span className="ds-chip" style={{ fontSize: 10, padding: "2px 8px" }}>
					{location}
				</span>
				<span style={{ width: 6, height: 6, borderRadius: "50%", background: prio }} />
				{starred && (
					<span style={{ color: "var(--amber)", fontSize: 12, marginLeft: "auto" }}>★</span>
				)}
			</div>
			{event && (
				<div
	...
	return (
		<div>
			<DSSubsection title="Modals & Dialogs">
				<div style={{ display: "flex", gap: 10 }}>
					<button className="ds-btn dark" onClick={() => setModal("add")}>
						Open Add Modal
					</button>
					<button className="ds-btn danger" onClick={() => setModal("confirm")}>
						Open Confirm Delete
					</button>
					<button className="ds-btn" onClick={() => setModal("info")}>
						Info Dialog
					</button>
				</div>
			</DSSubsection>
			...
			{modal && (
				<div className="ds-overlay" onClick={() => setModal(null)}>
					<div
						...
						style={{ width: modal === "confirm" ? 400 : 520 }}
					>
						{modal === "add" && (
							<>
								...
								>
									...
									<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
										...
										<div className="ds-field">
											...
											<input className="ds-input" placeholder="$0 – $0" />
										</div>
									</div>
								</div>
								<div className="ds-modal-ft">
									<button className="ds-btn" onClick={() => setModal(null)}>
										Cancel
									</button>
						...
						{modal === "confirm" && (
							<>
								<div style={{ padding: "24px 24px 16px", textAlign: "center" }}>
									...
									<div style={{ fontSize: 13, color: "var(--ink-2)", lineHeight: 1.5 }}>
										This will permanently remove the Automattic application and all associated
										notes, documents, and timeline events.
									</div>
								</div>
		...
		<div>
			<DSSubsection title="Tooltips">
				<div style={{ display: "flex", gap: 20, paddingTop: 10, paddingBottom: 40 }}>
					...
					<div style={{ position: "relative", display: "inline-flex" }}>
						<span className="ds-badge upcoming ds-tooltip-trigger" style={{ cursor: "default" }}>
							Upcoming
						</span>
						<div
							className="ds-tooltip"
					...
					{popover && (
						<div className="ds-popover" style={{ top: "100%", left: 0, marginTop: 6 }}>
							{["Edit application", "Duplicate", "Move to stage…", "Export as PDF"].map((item) => (
								<div key={item} className="ds-popover-item" onClick={() => setPopover(false)}>
									{item}
								</div>
							))}
