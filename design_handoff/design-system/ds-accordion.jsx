		...
		>
			<div
				...
				style={{
					...
					padding: isMinimal ? "14px 4px" : "14px 16px",
					cursor: "pointer",
					userSelect: "none",
				}}
			>
			...
			>
				<div
					style={{
						padding: isMinimal ? "0 4px 14px" : "0 16px 14px",
						fontSize: 12.5,
						color: "var(--ink-2)",
						lineHeight: 1.6,
					}}
...
function AccordionSection() {
	...
	const faqs = [
		{
			q: "How does JobDash track applications?",
			a: "You add an application once — company, role, link, status. JobDash auto-fills public details and surfaces follow-up reminders based on the stage.",
		},
		{
			q: "Can I import from LinkedIn or Indeed?",
			a: "Yes. Paste a job URL and we auto-extract title, company, salary range, and description into a draft application.",
		},
		{
			q: "Is my data private?",
			a: "Your data is yours. We never sell it or share it with employers. Export anytime as CSV or JSON.",
		},
		{
			q: "Does JobDash work with my browser/email?",
			a: "Optional Chrome extension captures applications from any site. Email forwarding to JobDash auto-creates entries.",
		},
	];
					...
					<div style={{ width: "100%" }}>
						{faqs.slice(0, 3).map((f, i) => (
							<AccordionItem
								...
								a={f.a}
								open={openMin === i}
								onToggle={() => setOpenMin(openMin === i ? -1 : i)}
								variant="minimal"
							/>
						))}
					</div>
