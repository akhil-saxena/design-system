	...
	return (
		<div style={{ position: "absolute", inset: 0, pointerEvents: open ? "auto" : "none" }}>
			...
			>
				...
				<div style={{ flex: 1, overflowY: "auto", padding: "0 18px 18px" }}>{children}</div>
			</div>
		</div>
	);
}
		...
		<div>
			<DSSubsection title="Bottom Sheet (mobile)">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 14, lineHeight: 1.5 }}>
					Modal drawer that slides up from the bottom. Snap points let users peek (quarter), inspect
					(half), or fully expand. Drag handle for affordance.
				</p>
				...
				<StateRow label="Action menu sheet">
					<div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
						{phoneFrame(
							<BottomSheetDemo open={openA} onClose={() => setOpenA(false)} snap={snapA}>
								...
								{[
									...
									>
										<span style={{ width: 22, color: a.c, fontSize: 14, textAlign: "center" }}>
											{a.i}
										</span>
						...
						<div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
							<button className="ds-btn" onClick={() => setOpenA(true)}>
								Open
							</button>
							<button className="ds-btn" onClick={() => setOpenA(false)}>
								Close
							</button>
							<div
								style={{
							...
							{["quarter", "half", "full"].map((s) => (
								<button
									key={s}
									className={`ds-btn ${snapA === s ? "amber" : ""}`}
									onClick={() => setSnapA(s)}
								>
									{s}
								</button>
					...
					<div style={{ display: "flex", gap: 16 }}>
						{phoneFrame(
							<BottomSheetDemo open={openB} onClose={() => setOpenB(false)} snap="full">
								...
								<div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
									{["Applied", "Interviewing", "Offer", "Rejected"].map((s, i) => (
										<div
											...
											className="ds-chip"
											style={{
												background: i < 2 ? "var(--amber-l)" : undefined,
												borderColor: i < 2 ? "rgba(245,158,11,.4)" : undefined,
											}}
										>
											{s}
										</div>
								...
								/>
								<button className="ds-btn amber" style={{ width: "100%" }}>
									Show 47 results
								</button>
							</BottomSheetDemo>,
						)}
