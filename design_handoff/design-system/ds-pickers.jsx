...
function DatePickerSection() {
	...
	const isInRange = (d) =>
		d && d >= Math.min(rangeStart, rangeEnd) && d <= Math.max(rangeStart, rangeEnd);

	return (
		<div>
			<DSSubsection title="Calendar with Time Picker">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 12, lineHeight: 1.5 }}>
					Full date + time selection. Events shown as amber dots. Click a day, then pick a time
					slot.
				</p>
				<div style={{ display: "grid", gridTemplateColumns: "280px 140px 1fr", gap: 16 }}>
						...
						>
							...
							{days.map((d, i) => {
								...
								return (
									<div
										key={i}
										onClick={() => d && d >= today && setSelectedDate(d)}
										...
										style={{
											...
											borderRadius: 8,
											fontSize: 12,
											cursor: d && d >= today ? "pointer" : "default",
											fontWeight: isSelected ? 700 : 400,
											background: isSelected
												? "var(--amber)"
												: isToday
													? "var(--amber-l)"
													: hoveredDay === d && d >= today
														? "var(--cream-2)"
														: "transparent",
											color: isSelected
												? "#fff"
					...
					<div
						className="glass"
						style={{ padding: 8, borderRadius: 14, maxHeight: 340, overflowY: "auto" }}
					>
						<div
							style={{
								fontFamily: "var(--mono)",
						...
						{times.map((t) => (
							<div
								...
								onMouseOver={(e) => {
									if (t !== selectedTime) e.currentTarget.style.background = "var(--cream-2)";
								}}
								onMouseOut={(e) => {
									if (t !== selectedTime) e.currentTarget.style.background = "";
								}}
							>
								{t}
							</div>
					...
					<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
						...
						<div className="ds-field">
							...
							<div className="ds-input-wrap">
								...
								<input
									className="ds-input"
									value={`Apr ${selectedDate}, 2026 at ${selectedTime}`}
									readOnly
			...
			<DSSubsection title="Date Range Picker">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 12, lineHeight: 1.5 }}>
					Click to set start, click again for end. Range highlights between.
				</p>
	...
	return (
		<div className="ds-field" ref={ref}>
			...
			<div
				...
				data-open={open}
				style={{ flexWrap: "wrap", minHeight: 38 }}
			>
			...
			{open && (
				<div className="ds-dropdown">
					...
					{filtered.map((o) => (
						<div
							key={o}
							className={`ds-dropdown-item ${selected.includes(o) ? "active" : ""}`}
							onClick={(e) => {
								e.stopPropagation();
								toggle(o);
							}}
						>
							<div
								className={`ds-checkbox ${selected.includes(o) ? "checked" : ""}`}
	...
	return (
		<div>
			<DSSubsection title="Editable Tag List">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 12, lineHeight: 1.5 }}>
					Type and press Enter to add. Click × to remove. Duplicates prevented.
				</p>
				<div
					className="glass"
					style={{
				...
				>
					{tags.map((t) => (
						<span
							key={t}
							className="ds-chip"
							style={{ background: "rgba(245,158,11,.1)", borderColor: "rgba(245,158,11,.2)" }}
						>
							{t}
							<span className="ds-chip-x" onClick={() => removeTag(t)}>
								×
							</span>
						</span>
					))}
			...
			<DSSubsection title="Multi-Select with Search">
				<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
					...
					<MultiSelect
						...
						options={["Remote", "Hybrid", "On-site", "Async-first", "Flexible hours", "4-day week"]}
						defaultSelected={["Remote", "Async-first"]}
					/>
				</div>
			</DSSubsection>
			...
			<DSSubsection title="Chip Variants">
				<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
					...
					<span
						className="ds-chip"
						style={{ background: "var(--ink)", color: "var(--cream)", borderColor: "var(--ink)" }}
					>
						Dark chip
					</span>
				</div>
			...
			<DSSubsection title="Badges">
				<div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
					...
					<span className="ds-badge" style={{ background: "var(--ink)", color: "var(--cream)" }}>
						New
					</span>
					<span
						className="ds-badge"
							...
							{i > 0 && (
								<div
									style={{
										width: 24,
										height: 1,
										background: i <= 2 ? "var(--ink-3)" : "var(--ink-5)",
										flexShrink: 0,
									}}
								/>
							)}
