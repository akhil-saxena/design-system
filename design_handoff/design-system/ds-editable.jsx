	...
	return (
		<DSSubsection title="Editable Table">
			<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 12, lineHeight: 1.5 }}>
				Double-click any cell to edit inline. Enter to commit, Escape to cancel. Cell highlights on
				focus.
			</p>
						...
						{data.map((row) => (
							<tr key={row.id}>
								{fields.map((f) => (
									<td
										key={f}
										onDoubleClick={() => startEdit(row.id, f)}
										style={{ cursor: "text", position: "relative" }}
									>
										{editing && editing.id === row.id && editing.field === f ? (
											<input
												ref={inputRef}
												className="ds-cell-edit"
												value={editVal}
												...
												onKeyDown={(e) => {
													...
													if (e.key === "Tab") {
														...
														if (fi < fields.length - 1) startEdit(row.id, fields[fi + 1]);
													}
												}}
											/>
										) : (
											<span
												className="ds-cell-display"
												style={{
													fontWeight: f === "company" ? 600 : 400,
													fontFamily: f === "salary" ? "var(--mono)" : "inherit",
													fontSize: f === "salary" ? 12 : 13,
												}}
											>
												{f === "stage" && (
													<span
								...
								<td>
									<button className="ds-icbtn" style={{ width: 24, height: 24, opacity: 0.4 }}>
										<svg
											viewBox="0 0 24 24"
											fill="none"
											stroke="currentColor"
...
function KanbanSection() {
	const [columns, setColumns] = React.useState({
		...
		applied: [
			{ id: "a1", company: "Automattic", role: "Senior SWE" },
			{ id: "a2", company: "Linear", role: "Product Engineer" },
		],
	...
	const onDrop = (targetCol) => {
		if (!dragging || dragging.col === targetCol) {
			setDragging(null);
			setOverCol(null);
			return;
		}
	...
	return (
		<DSSubsection title="Kanban with Cross-Column Drag">
			<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 12, lineHeight: 1.5 }}>
				Drag cards between columns to change stage. Column highlights when a card hovers over it.
			</p>
				...
				{Object.keys(colMeta).map((colKey) => (
					<div
						...
						style={{
							...
							padding: 8,
							background:
								overCol === colKey && dragging && dragging.col !== colKey
									? "rgba(245,158,11,.06)"
									: "rgba(0,0,0,.02)",
							border:
								overCol === colKey && dragging && dragging.col !== colKey
									? "1.5px dashed var(--amber)"
						...
						>
							<span
								style={{
									...
									height: 8,
									borderRadius: "50%",
									background: colMeta[colKey].color,
								}}
							/>
							<span style={{ fontFamily: "var(--display)", fontWeight: 700, fontSize: 12 }}>
								{colMeta[colKey].label}
							</span>
							<span
								style={{
									fontFamily: "var(--mono)",
						...
						{columns[colKey].map((item, idx) => (
							<div
								...
								style={{
									...
									gap: 8,
									alignItems: "center",
									opacity: dragging && dragging.col === colKey && dragging.idx === idx ? 0.4 : 1,
...
function RichTextSection() {
	...
	return (
		<DSSubsection title="Rich Text Mini Editor">
			<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 12, lineHeight: 1.5 }}>
				Toolbar with bold, italic, list, link. Uses contentEditable — suitable for notes and
				short-form text.
			</p>
			<div className="glass" style={{ borderRadius: 12, overflow: "hidden", maxWidth: 520 }}>
				...
				>
					...
					<ToolBtn
						...
						label="Numbered list"
						icon={
							<span style={{ fontFamily: "var(--mono)", fontSize: 10, fontWeight: 700 }}>1.</span>
						}
					/>
					<div
						style={{
				...
				<div
					ref={editorRef}
					contentEditable
					className="ds-richtext-body"
					onInput={updateCount}
					suppressContentEditableWarning
					dangerouslySetInnerHTML={{
						__html:
							"Reach out to <b>David Swanson</b> before the screening call. Key topics:<br><ul><li>Ask about the <b>Division structure</b> and team rotation</li><li>Clarify trial project expectations</li><li>Research <i>Jetpack vs WP.com</i> architecture split</li></ul>",
					}}
				></div>
				{/* Footer */}
				<div
				...
				>
					...
					<button className="ds-btn" style={{ fontSize: 10, padding: "3px 10px" }}>
						Save note
					</button>
				</div>
			</div>
