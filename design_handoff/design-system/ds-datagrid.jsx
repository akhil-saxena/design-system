...
const DG_STATUS = {
	applied: { l: "Applied", c: "upcoming" },
	interviewing: { l: "Interview", c: "done" },
	offer: { l: "Offer", c: "passed" },
	rejected: { l: "Rejected", c: "pending" },
};
const DG_PRIO = {
	high: "var(--red-vivid)",
...
function DataGridSection() {
	...
	const [widths, setWidths] = React.useState(() =>
		Object.fromEntries(DG_COLS.map((c) => [c.key, c.width])),
	);
		...
		<div>
			<DSSubsection title="Interactive Data Grid">
				<p style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 14, lineHeight: 1.5 }}>
					Sortable columns (click header), resizable (drag column edges), selectable rows with bulk
					actions. Pagination in footer.
				</p>
				<div className="glass" style={{ borderRadius: 14, overflow: "hidden" }}>
					...
					{sel.size > 0 && (
						...
						>
							...
							<button
								className="ds-btn ds-btn-xs ghost"
							...
							<thead>
								<tr>
									<th style={{ width: 40, textAlign: "center" }}>
										<div
											className={`ds-checkbox ${allSel ? "checked" : ""}`}
											style={{ width: 16, height: 16, margin: "0 auto", cursor: "pointer" }}
											onClick={toggleAll}
										>
											{allSel && (
												<svg
									...
									return (
										<tr key={row.id} style={{ background: s ? "rgba(245,158,11,.04)" : undefined }}>
											<td style={{ textAlign: "center" }}>
												<div
													className={`ds-checkbox ${s ? "checked" : ""}`}
													style={{ width: 16, height: 16, margin: "0 auto", cursor: "pointer" }}
											...
											<td>{row.role}</td>
											<td>
												<span className={`ds-badge ${st.c}`}>{st.l}</span>
											</td>
												...
												<span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
													<span
														style={{
															...
															height: 6,
															borderRadius: "50%",
															background: DG_PRIO[row.priority],
														}}
													/>
